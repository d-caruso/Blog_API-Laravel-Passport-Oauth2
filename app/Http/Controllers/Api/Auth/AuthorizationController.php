...
class AuthorizationController
{
     ...
     * @param  \Illuminate\Contracts\Auth\lGuard  $guard
    ...
    {
        // if the user is authenticated via API tokens
        if ($this->guard->check()) {
            // The user is already authenticated, so continue with the authorization flow
            return response()->json([
                'message' => 'User is authenticated, continue with authorization flow.'
            ]);
        }

        // Return a 401 Unauthorized response if user is not authenticated via API token
        return response()->json([
            'error' => 'Unauthorized: You need to be logged in to authorize this request.'
        ], 401);
    }
}
